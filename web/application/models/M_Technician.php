...
class M_Technician extends CI_Model {
    ...
    public function get()
    {
        $query = $this->db->where("t_id !=", 1)->get('h_technician');
        return $query->result();
    }
    ...
    public function update($data)
    {
        ...
        $this->db->update('h_technician', $data);
    ...
    {
        $query = $this->db->where("t_username=",$data['t_username'])->get("h_technician");
        if($query->num_rows() > 0) {

            // var_dump($query->row());
            $user = $query->result_array()[0];
            // if( password_verify($data['user_pass'], $user['user_pass']) ) {
            if( $data['t_password'] == $user['t_password'] ) {
